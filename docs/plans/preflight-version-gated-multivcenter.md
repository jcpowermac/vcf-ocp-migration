...
4. **No mechanism to persist the selected path across reconcile loops.** The reconciler is stateless — each `Reconcile()` call (line 126-153) walks `conditionOrder`, finds the first non-True condition, runs its handler, and returns. There is no shared in-memory state between phases. The version + gate check runs in `ensureInfrastructurePrepared()`, but two later handlers called on separate reconcile loops need the result:
   - `ensureMultiSiteConfigured()` line 429: must choose `infraMgr.AddTargetVCenterWithCRDModification()` (legacy) vs `infraMgr.AddTargetVCenter()` (native).
   - `ensureSourceCleaned()` line 718: must choose `infraMgr.RemoveSourceVCenterWithCRDModification()` (legacy) vs `infraMgr.RemoveSourceVCenter()` (native). Also conditionally calls `enableCVO()` at line 797, which must be skipped on the native path since CVO was never disabled.
...
6. **File organization for preflight logic is unspecified.** `ensureInfrastructurePrepared()` is currently ~120 lines. Adding version detection, gate detection, CSI PV checks, MHC/autoscaler checks, privilege validation, and advisory message composition will at least triple its size. The plan should decide whether this stays inline or goes into `internal/controller/preflight.go` (or `internal/openshift/preflight.go` for checks that are pure cluster reads).

7. **`ensureMultiSiteConfigured` idempotency guard.** This function (line 381-387) currently uses the condition message string (`"Waiting for vSphere pods to become ready"`) as a state marker to avoid re-running the entire configuration on re-reconcile. If the condition messages change (section 5 of the plan), this guard will break. **Proposed alternative:** replace the message-based guard with an actual state check — read the `Infrastructure` CR and check whether any of the target vCenter servers from `migration.Spec.FailureDomains` already exist in `infra.Spec.PlatformSpec.VSphere.VCenters[]`. If the target vCenters are already present, the configuration step completed on a prior reconcile and only the pod readiness wait needs to run. This is self-healing (correct regardless of message text), costs one API read that `ensureMultiSiteConfigured` already performs on the non-waiting path, and works identically on both the native and legacy paths.

8. **RESOLVED: CVO disable timing on the legacy path.** CVO must be paused because the Infrastructure CRD validations are temporarily removed. On the legacy path, disable CVO at the start of `ensureMultiSiteConfigured()` (before the first CRD mutation), keep it paused through `WorkloadMigrated`, and re-enable at the end of `ensureSourceCleaned()` (after the source vCenter is removed and the CRD is restored). This moves the disable out of `ensureInfrastructurePrepared()` (line 274) so preflight is purely non-disruptive, while keeping the same pause-through-completion semantics for the actual mutation window.
